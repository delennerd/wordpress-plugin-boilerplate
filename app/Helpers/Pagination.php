...
class Pagination
{
     ...
     */
    static function print( $query = null, $args = array(), $class = 'pagination' ) {
        global $wp_query;

        $query = $query != null ? $query : $wp_query;

        if ( ! isset( $args['total'] ) && $query->max_num_pages <= 1 ) {
            return;
        }
        ...
        $args = wp_parse_args(
            ...
            array(
                ...
                'mid_size'           => 2,
                // 'prev_next'          => true,
                'prev_text'          => __( '&laquo;', 'plugin-name' ),
                'next_text'          => __( '&raquo;', 'plugin-name' ),
                'type'               => 'array',
                'current'            => max( 1, get_query_var( 'paged' ) ),
                'screen_reader_text' => __( 'Posts navigation', 'plugin-name' ),
            )
        );
